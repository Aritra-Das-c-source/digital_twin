...
def _build_dark_calibration(runs: list[Path], configured_csv: Path, output_dir: Path) -> dict[str, Any]:
    configured = pd.read_csv(configured_csv)
    dark_ids = set(configured.loc[
        configured["sensor_coverage"].astype(str).str.upper().eq("NONE"), "station_id"
    ].astype(str))
    if not dark_ids:
        return {"dark_station_ids": [], "dwell": None, "corridor_residence": None, "history_run_ids": [run.name for run in runs]}

    # Existing DARK calibration logic is reused rather than reproduced.
    ...
    if not dwell_parts:
        raise ValueError("No completed DARK processing intervals were found in factory training runs")
...
def train_factory_model(
    *,
    ...
    try:
        ...
        calibration = _build_dark_calibration(runs, configured, staging / "calibration")
        ...
        manifest = {
            ...
            "factory": {"path": str(factory), "sha256": _sha256(factory)},
