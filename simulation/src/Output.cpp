    ...
    : outputDirectory(outputDirectory)
{
    ...
    if (!unitsFile || !stationEventsFile || !sensorReadingsFile || !manualChecksFile ||
        !inspectionResultsFile || !checkpointEventsFile)
    {
...
{
    stationEventsFile << eventKey(nextEventId++) << ',' << event.timestamp << ','
    ...
    stationEventsFile << ',';
    if (event.darkZoneId.has_value())
        stationEventsFile << *event.darkZoneId;
...
{
    checkpointEventsFile << "CE" << std::setw(6) << std::setfill('0') << nextCheckpointEventId++
                         << ',' << timestamp << ',' << eventType << ',' << stationKey(stationId)
                         << ',';
    if (unitId)
        checkpointEventsFile << unitKey(*unitId);
                                    ...
                                    const std::string& result)
{
    ...
    if (unitId)
        manualChecksFile << unitKey(*unitId);
