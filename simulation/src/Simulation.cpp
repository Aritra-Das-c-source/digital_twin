...
Simulation::Simulation(SimulationConfig value, std::filesystem::path directory,
		       std::string id)
    : config(std::move(value)), runId(std::move(id)), rng(config.randomSeed),
      sensorRng(config.randomSeed + 160),
      checkpointRng(config.randomSeed + 4242), stations(config.stations),
      observationPolicy(config.darkZones), output(std::move(directory))
{
	...
	for (const auto &c : config.checkpoints)
		checkpoints.push_back({c.stationId, c.id, c.type, c.progress,
				       c.reliability, c.falsePositiveRate,
				       c.identifiesUnit});
...
Time Simulation::sampleCycleTime(const Station &station, UnitId unit)
{
	...
	double multiplier = 1 + .45 * severity * severity +
			    (state.intermittentFaultActive ? .12 : 0);
	double cv =
	    station.cycleTimeCV *
	    (1 + 1.4 * severity + (state.intermittentFaultActive ? .55 : 0));
	...
	std::lognormal_distribution<double> dist(
	    std::log(station.meanCycleTime * multiplier) - sigma * sigma / 2,
	    sigma);
...
{
	double onset =
	    std::clamp((degradation[s.id].level - .10) / .70, 0., 1.);
	return onset * onset * (3 - 2 * onset);
}
Simulation::PhysicalActivity
Simulation::physicalActivity(const Station &s) const
{
	return s.state == StationState::PROCESSING
		   ? PhysicalActivity::PROCESSING_LOAD
		   : (s.state == StationState::BLOCKED && s.currentUnit
			  ? PhysicalActivity::HOLDING_UNIT
...
void Simulation::advanceDegradation(const Station &s)
{
	...
	if (d.scenario == DegradationScenario::GRADUAL)
		...
		inc =
		    .00007 +
		    std::max<std::int64_t>(
			0, static_cast<std::int64_t>(d.completedCycles) - 70) *
			.000007;
...
void Simulation::moveUnit(Station &from, Station &to, UnitId unit)
{
	if (const auto *zone = observationPolicy.enteredZone(from.id, to.id))
		emitStationEvent({currentTime,
				  StationEventType::DARK_ZONE_ENTERED, to.id,
				  unit, to.buffer.size() + 1, std::nullopt,
				  std::nullopt, std::nullopt, zone->id});
	if (const auto *zone = observationPolicy.exitedZone(from.id, to.id))
		emitStationEvent({currentTime,
				  StationEventType::DARK_ZONE_EXITED, to.id,
				  unit, to.buffer.size() + 1, std::nullopt,
				  std::nullopt, std::nullopt, zone->id});
	to.buffer.push(unit);
	...
	emitStationEvent({currentTime, StationEventType::UNIT_ARRIVED, to.id,
			  unit, to.buffer.size(), std::nullopt, std::nullopt,
...
void Simulation::tryUnblockUpstream(Station &station)
{
	...
	if (up.state == StationState::BLOCKED && up.currentUnit &&
	    canAcceptUnit(station)) {
...
UnitId Simulation::createUnit()
{
	...
	units.emplace(id, Unit{id,
			       id % 5 == 0 ? "MODEL_B" : "MODEL_A",
			       id % 7 == 0 ? "BATCH_02" : "BATCH_01",
			       {}});
...
bool Simulation::unitHasDefect(UnitId id, const std::string &type) const
{
	...
	return std::any_of(d.begin(), d.end(),
			   [&](const UnitDefect &x) { return x.type == type; });
}
std::vector<const DefectEffectConfig *>
Simulation::effectsFor(UnitId id, StationId station) const
{
...
{
	for (const auto &def : config.defects) {
		if (std::find(def.introductionStations.begin(),
			      def.introductionStations.end(),
			      s.id) == def.introductionStations.end() ||
		    unitHasDefect(id, def.type))
			continue;
		double probability = std::clamp(
		    def.baseProbability +
			degradation[s.id].level * def.degradationSensitivity,
		    0., 1.);
		std::bernoulli_distribution introduce(probability);
		if (introduce(rng))
			units.at(id).defects.push_back(
			    {def.type, s.id, currentTime});
	}
}
void Simulation::emitObservableData(const Station &s, UnitId id)
{
	if (s.archetype == "MANUAL" &&
	    observationPolicy.shouldExportManualCheck(s.id)) {
		double fail = .015;
		for (auto e : effectsFor(id, s.id))
			if (auto found = e->manualCheckFailProbabilities.find(
				"VISUAL_ALIGNMENT");
		...
		output.writeManualCheck(
		    currentTime, s.id,
		    observationPolicy.zoneFor(s.id) ? std::optional<UnitId>{}
						    : std::optional<UnitId>{id},
			...
			if (unitHasDefect(id, def.type))
				for (const auto &e : def.effects)
					if (e.stationId == s.id &&
						...
						if (d(rng)) {
							found = true;
							output.writeInspectionResult(
							    currentTime, s.id,
							    id, def.type,
							    e.inspectionSeverity,
							    "FAIL");
						}
					}
		if (!found)
			output.writeInspectionResult(currentTime, s.id, id, "",
						     std::nullopt, "PASS");
	}
}
...
{
	std::uniform_int_distribution<Time> jitter(
	    s.sensorCoverage == "HIGH" ? -1200 : -3500,
	    s.sensorCoverage == "HIGH" ? 1200 : 3500);
	return std::max<Time>(1000,
			      sensorSamplingInterval(s) + jitter(sensorRng));
}
void Simulation::emitSensorSample(const Station &s)
{
	...
	if (processing && s.currentUnit)
		for (auto e : effectsFor(*s.currentUnit, s.id)) {
			if (auto i = e->sensorMeanShifts.find("VIBRATION");
			    i != e->sensorMeanShifts.end())
				vibrationShift += i->second;
			if (auto i = e->sensorMeanShifts.find("TEMPERATURE");
			    i != e->sensorMeanShifts.end())
				tempShift += i->second;
		}
	...
	auto respond = [&](double value, double target, double rate,
			   double noise) {
		std::normal_distribution<double> n(0, noise);
		return std::max(0.,
				value + rate * (target - value) + n(sensorRng));
	};
	double v = processing
		       ? 1.15
		       : (activity == PhysicalActivity::HOLDING_UNIT ? .48
								     : .18),
	       t = processing
		       ? 61
		       : (activity == PhysicalActivity::HOLDING_UNIT ? 45 : 39),
	       c = processing
		       ? 14.5
		       : (activity == PhysicalActivity::HOLDING_UNIT ? 4.5
								     : 2.2);
	sensor.vibration =
	    respond(sensor.vibration,
		    v + drift * 1.6 + vibrationShift +
			(state.intermittentFaultActive ? .35 : 0),
		    .65, .035);
	sensor.temperature =
	    respond(sensor.temperature, t + drift * 13 + tempShift, .12, .16);
	sensor.current = respond(sensor.current, c + drift * 1.3, .85, .12);
	output.writeSensorReading(currentTime, s.id, "VIBRATION",
				  sensor.vibration, "g");
	output.writeSensorReading(currentTime, s.id, "TEMPERATURE",
				  sensor.temperature, "C");
	if (s.sensorCoverage == "HIGH") {
		output.writeSensorReading(currentTime, s.id, "CURRENT",
					  sensor.current, "A");
	}
}
void Simulation::handleSensorSample(const SimEvent &e)
{
	...
	emitSensorSample(s);
	events.push({currentTime + nextSensorSamplingInterval(s),
		     SimEventType::SENSOR_SAMPLE, s.id, 0});
}
void Simulation::tryStartProcessing(Station &s)
{
	if (s.state == StationState::PROCESSING ||
	...
	emitStationEvent({currentTime, StationEventType::PROCESSING_STARTED,
			  s.id, id, s.buffer.size(), prior,
			  StationState::PROCESSING, s.currentCycleTime,
			  std::nullopt});
	scheduleCheckpointEvents(s, id);
	events.push({currentTime + s.currentCycleTime,
		     SimEventType::PROCESSING_COMPLETE, s.id, id});
}
void Simulation::scheduleCheckpointEvents(const Station &s, UnitId id)
{
	for (std::size_t i = 0; i < checkpoints.size(); ++i) {
		const auto &cp = checkpoints[i];
		if (cp.stationId != s.id ||
		    !observationPolicy.shouldExportCheckpoint(s.id))
			continue;
		Time nominal =
		    currentTime +
		    static_cast<Time>(std::round(cp.nominalProgressFraction *
						 s.currentCycleTime));
		std::bernoulli_distribution occurs(cp.readReliability);
		if (occurs(checkpointRng))
			checkpointEvents.push(
			    {nominal, SimEventType::CHECKPOINT_RECORDED, s.id,
			     id, i});
	}
}
void Simulation::flushCheckpointEvents()
{
	while (!checkpointEvents.empty()) {
		...
		output.writeCheckpointEvent(
		    e.timestamp,
		    cp.checkpointType == "RFID" ? "RFID_CHECKPOINT"
						: "POWER_DRAW",
		    e.stationId,
		    observationPolicy.shouldExposeCheckpointIdentity(
			e.stationId, cp.identifiesUnit)
...
void Simulation::handleProcessingComplete(const SimEvent &e)
{
	...
	emitStationEvent({currentTime, StationEventType::PROCESSING_COMPLETED,
			  s.id, e.unitId, s.buffer.size(),
			  StationState::PROCESSING, std::nullopt,
			  s.currentCycleTime, std::nullopt});
...
void Simulation::run()
{
	...
	output.writeRunMetadata(runId, config.randomSeed, config.duration,
				stations.size(), nextUnitId - 1);
}
